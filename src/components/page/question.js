...
class Question extends Component {
  constructor (props) {
    ...
    const { api, game: { seconds } } = this.props
  ...
  componentDidMount () {
    const { _seconds, form } = this.state

    form.seconds = _seconds || ''
  ...
  handleChange (event) {
    ...
    const { answer: _a, question: _q, seconds: _s } = form
  ...
  async handleSubmit (event) {
    ...
    const { _api, errors, form: { answer, picture, question, seconds } } = this.state
  ...
  render () {
    ...
    const { errors, form: { answer, picture, question, seconds } } = this.state
    ...
    return (
      <GameContainer game={game}>
        ...
        {(picture ? <ImgContainer onClick={handleClick} picture={picture} /> : null)}
      </GameContainer>
    )
  }
}
